        ...
        requireActivity().runOnUiThread {
            ...
            b.predictiveStatus.setTextColor(
                when (snapshot.predictiveLevel) {
                    NetworkGuardianEngine.Level.STABLE -> Color.parseColor("#3DDC97")
                    NetworkGuardianEngine.Level.WATCH -> Color.parseColor("#F0A500")
                    NetworkGuardianEngine.Level.HIGH -> Color.parseColor("#FF8C42")
                    NetworkGuardianEngine.Level.CRITICAL -> Color.parseColor("#E85D5D")
                }
            )
            ...
            b.bwBlock.text = buildString {
                appendLine("BANDWIDTH / PREDICTIVE (NetworkMonitor_v2)")
                ...
                appendLine("Storm score: ${snapshot.stormScore}/100  (heuristic deauth pressure)")
            }.trimEnd()

            b.mitigation.text = snapshot.mitigation

            b.alertsBlock.text = if (snapshot.alerts.isEmpty()) {
                "No deauthentication / disconnect anomalies recorded yet.\nSystem advice: monitor active."
            } else {
